...
public sealed partial class Subject42OrbitalOperationsTests
{
    ...
    [Category("Core")]
    [Test]
    public void RejectedCommand_PreservesEveryField()
    {
        foreach (string kind in new[] { "occupied install", "bad ring install", "bad mount install", "bad kind install", "occupied move", "bad module move", "bad ring move", "bad mount move", "mount cap", "speed cap", "power cap", "power overflow", "core cap", "link damage", "ring cap", "module allocator limit", "malformed" })
        {
            TestContext.Progress.WriteLine("RejectedCommand_PreservesEveryField: " + kind);
            AssertRejectedCommand_PreservesEveryField(kind);
        }
    }

    private void AssertRejectedCommand_PreservesEveryField(string kind)
    {
    ...
    [Category("Core")]
    [Test]
    public void LinkPair_RejectedAtomically()
    {
        foreach (string reason in new[] { "same", "occupied", "missing", "allocator" })
        {
            TestContext.Progress.WriteLine("LinkPair_RejectedAtomically: " + reason);
            AssertLinkPair_RejectedAtomically(reason);
        }
    }

    private void AssertLinkPair_RejectedAtomically(string reason)
    {
        var state = OrbitalRunState.CreateDefault(1);
        state.AddMount(1, out _); state.AddMount(1, out _);
