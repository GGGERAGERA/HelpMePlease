...
public sealed partial class Subject42AuthoredUiTests
{
    ...
    [UnityTearDown]
    public IEnumerator RestorePreferencesAndPlayMode() => new Subject42FinalBossFlowTests().CleanupPlayMode();
    ...
    private static IEnumerator ExerciseProductionUi()
    {
        ...
        yield return Await(() => One<OrbitalStationRuntime>() != null && One<OrbitalStationRuntime>() != pauseStation && One<OrbitalStationRuntime>().IsInitialized && !SceneTransitionOverlay.IsTransitioning);
        ...
        yield return Await(() => One<OrbitalStationRuntime>() != null && One<OrbitalStationRuntime>() != previousStation && One<OrbitalStationRuntime>().IsInitialized && !SceneTransitionOverlay.IsTransitioning);
        ...
        yield return Await(() => One<OrbitalStationRuntime>() != null && One<OrbitalStationRuntime>() != deadStation && One<OrbitalStationRuntime>().IsInitialized && !SceneTransitionOverlay.IsTransitioning);
        Time.timeScale = 0; AssertEvents();
    ...
    private static IEnumerator StartRun()
    {
        ...
        yield return Await(() => SceneManager.GetActiveScene().name == "MVP" && !SceneTransitionOverlay.IsTransitioning && One<OrbitalStationRuntime>() != null && One<OrbitalStationRuntime>().IsInitialized && One<ProductionExplorationSectorController>() != null);
    }
    ...
    private static void PointerClick(Button button, bool verifyRaycast = false)
    {
        ...
        if (verifyRaycast)
        {
            ...
            Assert.That(hits[0].gameObject.GetComponentInParent<Button>(), Is.SameAs(button),
                button.name + " is occluded; transitioning=" + SceneTransitionOverlay.IsTransitioning +
                "; raycast order=" + string.Join(" -> ", hits.Take(5).Select(hit => HierarchyPath(hit.gameObject.transform))));
        }
