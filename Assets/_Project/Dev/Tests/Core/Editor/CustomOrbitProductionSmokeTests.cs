...
public sealed class CustomOrbitProductionSmokeTests
{
    ...
    public IEnumerator ProductionCreationQueue_Modules_AndSectorRestore()
    {
        ...
        try
        {
            ...
            int casinoRingId = station.State.PendingCasinoRingId;
            Assert.That(casinoRingId, Is.Not.Zero, "The pre-run bonus is now owned by this run");
            Assert.That(station.State.IsPending(station.State.FindRing(casinoRingId)), Is.True);
            Assert.That(OrbitalSlotMachine.Pending, Is.EqualTo(OrbitalSlotSymbol.None),
                "Transferred bonus must not remain available to a replacement run");
            Draw(station, false);
            yield return WaitDrawing(station);
            Assert.That(station.State.PendingRingCount, Is.EqualTo(1));
            Assert.That(station.Rings.Count, Is.EqualTo(1));
            Assert.That(Time.timeScale, Is.Zero);
            Assert.That(station.State.PendingCasinoRingId, Is.EqualTo(casinoRingId));
            Assert.That(station.State.IsPending(station.State.FindRing(casinoRingId)), Is.True,
                "Confirming the starting ring must not consume the pending casino path");
            Assert.That(OrbitalSlotMachine.Pending, Is.EqualTo(OrbitalSlotSymbol.None));
            Draw(station, true);
            yield return null;
            Assert.That(station.Rings.Count, Is.EqualTo(2));
            Assert.That(station.State.PendingRingCount, Is.Zero);
            Assert.That(station.State.PendingCasinoRingId, Is.Zero,
                "Confirming the earned path completes its run-owned pending marker");
            Assert.That(station.State.FindRing(casinoRingId).CustomPath, Is.Not.Null.And.Not.Empty);
            Assert.That(station.State.IsPending(station.State.FindRing(casinoRingId)), Is.False);
            Assert.That(OrbitalSlotMachine.Pending, Is.EqualTo(OrbitalSlotSymbol.None));
            Assert.That(Time.timeScale, Is.EqualTo(1f));
            Debug.Log("CustomOrbit PASS 4: pre-run casino bonus transferred once; sequential drawing clears run-owned marker only after second Confirm.");
        }
        finally
        {
