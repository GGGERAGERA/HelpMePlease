...
Открыть `Assets/_Project/Dev/Labs/F1/EnemyGallery.unity`, нажать Play.
...
`Tools > Subject42 > Dev > F1 Inspector > Enemy Gallery > Refresh Enemy Gallery` обновляет production references
и варианты из `prefabs/Enemies/PreparedVariants`. Сохраняет позиции и зону B.

`Tools > Subject42 > Dev > F1 Inspector > Enemy Gallery > Refresh Enemy Visual Candidates` обновляет displays из
...
`Tools > Subject42 > Dev > F1 Inspector > Enemy Gallery > Validate Enemy Gallery (3 minute Play Mode)` запускает
