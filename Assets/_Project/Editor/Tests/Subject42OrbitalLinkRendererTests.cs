...
public sealed class Subject42OrbitalLinkRendererTests {
 ...
 try{
 Assert.That(station.AddMount(1,out _),Is.True);Assert.That(station.AddMount(1,out _),Is.True);Assert.That(station.InstallLinkPair(1,1,1,2,out _),Is.True);var pair=station.State.ResolveLinkPairs().Single();var first=station.Modules.Single(m=>m.StableModuleId==pair.First);var second=station.Modules.Single(m=>m.StableModuleId==pair.Second);
 ...
 var line=Line(station);int instance=line.GetInstanceID();Assert.That(line.widthMultiplier,Is.EqualTo(.045f));Assert.That(line.startColor.r,Is.EqualTo(.1f).Within(1f/255));Assert.That(line.startColor.g,Is.EqualTo(.85f).Within(1f/255));Assert.That(line.startColor.b,Is.EqualTo(.55f).Within(1f/255));Assert.That(line.startColor.a,Is.EqualTo(.35f).Within(1f/255));Assert.That(line.endColor,Is.EqualTo(line.startColor));Assert.That(line.sortingLayerName,Is.EqualTo("Player"));Assert.That(line.sortingOrder,Is.EqualTo(13));Assert.That(line.sharedMaterial,Is.Not.Null);
